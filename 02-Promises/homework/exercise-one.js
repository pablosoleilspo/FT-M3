...
.then(st => { blue(st)}) //tambien se puede hacer .then(blue)
}
...
// el otro then toma el valor del return anterior 
.then(st3 => {
blue(st3)
console.log('done')
})
...
function problemE () {
  ...
  .then(st3 => {
    blue(st3)
    return promisifiedReadFile('poem-one/stanza-04ff.txt')
  })
  // el otro then toma el valor del return anterior 
  .then(st4 => {
  blue(st4)
...
function problemF () {
  ...
  //esto podria ser otra opcion para que cuando siempre termine muestre done
  // .finally(() => {
  //   console.log('done')
  // })
}
